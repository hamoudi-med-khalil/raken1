...
const saveCartFromStorage = (cart) => {
    localStorage.setItem("cart", JSON.stringify(cart))
}

// Async Fetch cart for a user or guest
export const fetchCart = createAsyncThunk("cart/fetchCart", async ({ userId, guestId }, { rejectWithValue }) => {
    try {
        const response = axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/cart`,
            {
...
const cartSlice = createSlice({
    ...
    extraReducers: (builder) =>{
        ...
        .addCase(fetchCart.fulfilled, (state, action) => {
            ...
            saveCartFromStorage(action.payload)
        })
        ...
        .addCase(mergeCart.rejected, (state, action) => {
            state.loading = false;
            state.error = action.payload?.message || "failed to merge item quantity"
        })
    }
})
